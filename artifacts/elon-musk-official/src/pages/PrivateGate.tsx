...
export default function PrivateGate() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (pw === "Elon2026") {
      window.location.href = "/crypto-endowment";
    } else {
      setError(true);
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        ...
        <div className="flex justify-center mb-12">
          <svg viewBox="0 0 120 22" className="h-6 fill-white opacity-90" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="text-center mb-8">
          <h1 className="text-xl font-medium tracking-tight text-white mb-2">Restricted Access</h1>
          <p className="text-sm text-white/40">This page is private. Enter your access code.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input
            ...
            className={`w-full bg-[#111] border ${error ? "border-red-500" : "border-[#333]"} rounded-lg px-4 py-3 text-sm text-white placeholder-white/30 focus:outline-none focus:border-white/50 transition-colors`}
          />
          {error && <p className="text-xs text-red-400 text-center">Invalid code. Try again.</p>}
          <button
            type="submit"
